...
namespace ledpixel {
    ...
    typedef boost::geometry::model::d3::point_xyz<double, boost::geometry::cs::cartesian> Point;
    ...
    class LEDPixel {
        public:
            ...
            LEDPixel(Point coords, Point rot, DMXAddr addr, bool ignore, float sampleRadius)
                : mCoords{ coords }
                , mRot{ rot }
                , mAddr{ addr }
                , mIgnore{ ignore }
            ...
            friend std::ostream &operator << (std::ostream &out, const LEDPixel<ColorT> &L)
            {
    ...
    class LEDPixels {
        public:
            ...
            LEDPixels(std::vector<LEDPixelT> pixels, std::map<std::string, std::vector<size_t>> groups)  // pixels and groups
            {
                MultiPoint points;
                size_t index = 0;
                ...
                if (!EVP_DigestUpdate(m_context, hashInStr.str().c_str(), hashInStr.str().length())) {
            ...
            LEDPixels(std::tuple<std::vector<LEDPixelT>, std::map<std::string, std::vector<size_t>>> pixelsAndGroups)  // pixels and groups as tuple
            ...
            LEDPixels(LEDPixels &targetLEDPixels, bool useCache, double spacing)  // auto grid from target for sampler
            ...
            size_t pixelGroupsToString(std::shared_ptr<LEDPixelT> pixel, std::string &groupsStr) const
            {
            ...
            friend std::ostream &operator << (std::ostream &out, const LEDPixels<LEDPixelT> &L)
            {
        ...
        private:
            ...
            std::tuple<std::vector<LEDPixelT>, std::map<std::string, std::vector<size_t>>> pixelsFromCSV(std::string filename) const
            {
    ...
    class LEDPixelsSampler {
        public:
            ...
            LEDPixelsSampler(LEDPixelsT &targetLEDPixels, LEDPixelsT sourceLEDPixels, bool useCache)
            ...
            void sample()
            {
                ...
                for (auto const& [targetPixel, sources] : mSamplerMap) {
                    ...
                    MixColor targetColor({
                        std::sqrt(std::accumulate(mixR.begin(), mixR.end(), 0.0) / (double)mixR.size()),
                        std::sqrt(std::accumulate(mixG.begin(), mixG.end(), 0.0) / (double)mixG.size()),
                        std::sqrt(std::accumulate(mixB.begin(), mixB.end(), 0.0) / (double)mixB.size())
                    });
                ...
                : LEDPixelsOutput<LEDPixelsT, ColorToOutput>::LEDPixelsOutput( ledPixels )
            {
                ...
                for( std::shared_ptr<LEDPixelT> pixel: mLEDPixels.mPixels ) {
                    ...
                    if (mUniversesToBuffers.count(addr.mUniverse) < 1) {
                        ...
                        BOOST_LOG_TRIVIAL(info) << "LEDPixelsOutputOLA: Created universe " << addr.mUniverse << " buffer " << i;
                    }
                }

                ola::client::StreamingClient::Options olaClientOptions = ola::client::StreamingClient::Options();
                olaClientOptions.auto_start = false;
                mOLAClient = std::unique_ptr<ola::client::StreamingClient>(new ola::client::StreamingClient(olaClientOptions));
                if (!mOLAClient->Setup()) {
            ...
            bool sendData() override
            {
                ...
                for (auto const& [universe, i] : mUniversesToBuffers) {
                    if (!mOLAClient->SendDmx(universe, mBuffers[i])) {
                        res = false;
                        BOOST_LOG_TRIVIAL(error) << "LEDPixelsOutputOLA: Send DMX failed universe " << universe;
                    }
                }
                return res;
            }
