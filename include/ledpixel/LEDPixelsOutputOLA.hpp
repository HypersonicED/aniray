...
class LEDPixelsOutputOLA : LEDPixelsOutput<LEDPixelsT, ColorToOutput> {
    public:
        ...
        LEDPixelsOutputOLA( LEDPixelsT &ledPixels ) // NOLINT(cppcoreguidelines-pro-type-member-init,hicpp-member-init)
            : LEDPixelsOutput<LEDPixelsT, ColorToOutput>::LEDPixelsOutput( ledPixels )
        {
            ...
            mOLAClient = std::make_unique<ola::client::StreamingClient>(ola::client::StreamingClient(olaClientOptions));
            if (!mOLAClient->Setup()) {
